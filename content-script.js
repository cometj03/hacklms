...
(async () => {
    ...
    // wait for videoIframe loaded
    // TODO: 영상 강의가 아닌 경우 videoIframe이 없음 -> 종료시키기
    ...
    if (DEBUG) console.log('videoSrc', videoSrc); // videoSrc ex) https://commons.ssu.ac.kr/em/69bd616f92eb6?startat=0.00&endat=0.00&TargetUrl=https%3A%2F%2Fcanvas.ssu.ac.kr%2Flearningx%2Fapi%2Fv1%2Fcourses%2F44036%2Fsections%2F0%2Fcomponents%2F794113%2Fprogress%3Fuser_id%3D20222904%26content_id%3D69bd616f92eb6%26content_type%3Dmovie&sl=1&pr=1&mxpr=1.00&lg=ko
    const targetUrl = new URL(videoSrc).searchParams.get('TargetUrl');
    if (DEBUG) console.log('targetUrl', targetUrl); // targetUrl ex) https://canvas.ssu.ac.kr/learningx/api/v1/courses/44036/sections/0/components/794113/progress?user_id=20222904&content_id=69bd616f92eb6&content_type=movie
    ...
    completeBtn.onclick = async () => {
        ...
        if (c.attendance_data?.completed) {
            if (!confirm('이미 학습이 완료되었습니다. 그럼에도 실행하시겠습니까?'))
                return;
        }

        let time = c.attendance_data?.last_at ?? 0;
        let delta = 60;
        do {
            time += delta;
            ...
            const c = await check();
            console.log(`${time / duration * 100}% 완료`, {completed: c.attendance_data?.completed, time, delta});
        } while (time < duration);
